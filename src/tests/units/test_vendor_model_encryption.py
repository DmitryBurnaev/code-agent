...
class TestVendorEncryption:
    ...
    @patch("src.db.models.get_app_settings")
    def test_decrypted_api_key_raises_on_decryption_error(self, mock_get_settings: MagicMock) -> None:
        """Test that decrypted_api_key raises error on decryption failure."""
        # Mock settings to cause decryption error
        mock_settings = MagicMock(spec=AppSettings)
        mock_settings.vendor_encryption_key.get_secret_value.side_effect = ValueError("Decryption error")
        mock_get_settings.return_value = mock_settings
