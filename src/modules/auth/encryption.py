...
class VendorKeyEncryption:
    ...
    Each encryption operation uses random nonce for security.
    """

    # AES-256 requires 32 bytes key
    KEY_SIZE = 32
    # GCM nonce size (12 bytes are recommended for AES-GCM)
    NONCE_SIZE = 12
    ...
    def __init__(self, secret_key: SecretStr) -> None:
        ...
        """
        self._secret_key = hashlib.sha256(secret_key.get_secret_value().encode()).digest()
